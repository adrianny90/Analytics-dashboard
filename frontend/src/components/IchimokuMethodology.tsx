...
function H2({ id, children }: { id?: string; children: ReactNode }) {
  return (
    <h2 id={id} className="pt-6 text-xl font-semibold text-white">
      {children}
    </h2>
  );
}
...
function H3({ children }: { children: ReactNode }) {
  return <h3 className="pt-2 text-base font-semibold text-white/90">{children}</h3>;
}
...
const code = (s: string) => (
  <code className="rounded bg-white/10 px-1 py-0.5 text-white/90">{s}</code>
);
