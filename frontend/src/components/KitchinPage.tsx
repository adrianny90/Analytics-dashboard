...
const HEADERS: { key: string; label: string; span: number }[] = [
  { key: "wzrost", label: "Wzrost", span: 2 },
  { key: "spowolnienie", label: "Spowolnienie", span: 1 },
  { key: "recesja", label: "Recesja", span: 2 },
  { key: "ozywienie", label: "Ożywienie", span: 1 },
];

const CATEGORY_LABEL: Record<string, string> = {
  bonds: "Obligacje (US / Japonia / Europa)",
  stocks: "Akcje (US / Japonia / Europa)",
  commodities: "Surowce",
};
...
export function KitchinPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-12">
      <h1 className="text-2xl font-semibold">Cykl Kitchina</h1>
      <p className="mt-1 text-sm text-white/50">
        Rentowności US 10Y/2Y i obligacje Japonii/Niemiec/UK, giełda USA/Japonii/Europy oraz koszyk surowcowy
        (indeks + miedź + ropa + złoto), zestawione wg klasycznej rotacji obligacje-akcje-surowce, żeby oszacować,
        w której fazie cyklu koniunkturalnego prawdopodobnie jesteśmy.
      </p>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        ...
        {snapshot?.updated_at && (
          <span className="text-xs text-white/40">Ostatnia aktualizacja: {new Date(snapshot.updated_at).toLocaleString()}</span>
        )}
      </div>

      {error && <p className="mt-4 text-fall">Nie udało się pobrać danych: {error}</p>}
