  ...
  return (
    <section className="mt-6 rounded-xl border border-white/10 p-4">
      ...
      <p className="mt-3 text-xs text-white/40">
        {t(
          "RSI(14), wygładzanie Wildera - dokładnie to samo co panel RSI na wykresie. Skan sprawdza każdą spółkę w tabeli i zostawia tylko te, których RSI na wybranym interwale mieści się w zakresie (np. D1, min 10, max 40). Wartości RSI policzone niedawno przez Start albo godzinne odświeżenie są reużywane; w przeciwnym razie ten interwał jest pobierany dla całej listy (M1 zawsze wymaga własnego pobrania za pierwszym razem).",
          "RSI(14) with Wilder smoothing – exactly the same as the RSI panel on the chart. The scan checks every stock in the table and keeps only those whose RSI on the chosen timeframe falls within the range (e.g. D1, min 10, max 40). RSI values calculated recently by Start or by the hourly refresh are reused; otherwise that timeframe is downloaded for the whole list (M1 always needs its own download the first time).",
          "RSI(14) mit Wilder-Glättung – exakt derselbe wie im RSI-Panel des Charts. Der Scan prüft jede Aktie in der Tabelle und behält nur diejenigen, deren RSI im gewählten Zeitrahmen im Bereich liegt (z. B. D1, min 10, max 40). Kürzlich durch Start oder die stündliche Aktualisierung berechnete RSI-Werte werden wiederverwendet; andernfalls wird dieser Zeitrahmen für die gesamte Liste geladen (M1 muss beim ersten Mal immer separat geladen werden).",
        )}
      </p>
    </section>
  );
